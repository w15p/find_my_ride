            ...
            with sync_playwright() as p:
                ...
                for i, url in enumerate(urls, 1):
                    try:
                        ...
                        # NOTE: FB DOM price re-poll was removed 2026-07-20.
                        # _read_dom_price grabs the first price-like element,
                        # but the FB item page renders many (the listing, its
                        # USD-converted approximate for a US egress, and
                        # Today's-picks tile prices). It concatenated multiple
                        # numbers ("850" + "1,100" -> 8501100) and mislabelled
                        # currency, corrupting good prices. FB price refresh
                        # needs the Relay/JSON `listing_price` extraction the
                        # scraper uses, not DOM scraping - deferred. The
                        # update_price helper, schema, API and UI arrow stay
                        # in place for eBay (clean structured API prices).
